...
from infractl.config import ClusterConfig, Environment, Paths
from infractl.services import ArgoCDService, AWSService, EksctlService, GitService, KubernetesService
...
def generate_import_yaml(config: ClusterConfig, region: str) -> str:
    ...
    header = f"""# Auto-generated by infractl bootstrap
...
def load_permission_boundary(policy_path: Path, account_id: str) -> dict[str, Any]:
    """Load and customize the permission boundary policy."""
    if not policy_path.exists():
        console.print(f"[red]Permission boundary file not found: {policy_path}[/red]")
        raise typer.Exit(1)
...
@app.command()
def dev(
    cluster_name: Annotated[str, typer.Option("--cluster-name", "-n", help="EKS cluster name")] = "stack-ai-dev",
...
@app.command()
def prod(
    cluster_name: Annotated[str, typer.Option("--cluster-name", "-n", help="EKS cluster name")] = "stack-ai-prod",
    region: Annotated[str, typer.Option("--region", "-r", help="AWS region")] = "us-east-1",
    skip_cluster_create: Annotated[bool, typer.Option("--skip-cluster-create", help="Skip EKS cluster creation")] = False,
    skip_git_push: Annotated[bool, typer.Option("--skip-git-push", help="Skip git commit and push")] = False,
    skip_sync: Annotated[bool, typer.Option("--skip-sync", help="Skip ArgoCD sync orchestration")] = False,
